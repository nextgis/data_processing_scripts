...
for file in files:
    ...
    if i == 1:
        ...
        cmd = 'gdal_translate -co SPARSE_OK=TRUE -q -of GTiff {compress_settings}  {file_current} {file_result} '.format(file_result=file_result, file_current=file_current, compress_settings=compress_settings)
        print cmd
        os.system(cmd)
    #print file
    if i > 1:
        ...
        file_current = os.path.join(dirpath,file)
        cmd = 'gdal_merge.py  -co SPARSE_OK=TRUE -q -of GTiff {compress_settings} -v -o {file_result} {file_prev} {file_current}'.format(file_result = file_result, file_prev=file_prev, file_current=file_current, compress_settings=compress_settings)
