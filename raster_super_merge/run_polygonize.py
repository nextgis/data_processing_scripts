...
for file in files:
    ...
    #remove jpg artifacts from mask
    cmd = 'gdal_translate -b 4 -ot Byte {file_current} /vsistdout/ | nearblack -o /vsistdout/ -white /vsistdin/ | gdal_translate -ot Byte /vsistdin/ {path}'.format(file_current=file_current,path=os.path.join(stack_dir,str(i)) + '_mask.tiff')
    #gdal_polygonize.py -8  {file_current} -b 4 -f GPKG {path}'.format(file_current=file_current,path=os.path.join(stack_dir,str(i)) + '_mask.gpkg')
    #cmd = '
    '''
    gdal_translate -b 4 -ot Byte ../../../generated_o/jpg25/2017-11-03-01_orto.tif /vsistdout/ | nearblack -o /vsistdout/ -white /vsistdin/ |gdal_nearblack.py -8  ../../../generated_o/jpg25/2017-11-03-01_orto.tif -b 4 -f GPKG ../../../generated_o/jpg25/stack/1_mask.gpkg
    '''
    print cmd
    quit()
    os.system(cmd)
    #polygonize vector mask
    cmd = 'gdal_polygonize.py -8  {file_current} -b 4 -f GPKG {path}'.format(file_current=file_current,path=os.path.join(stack_dir,str(i)) + '_mask.gpkg')
    print cmd
    os.system(cmd)
    quit()
